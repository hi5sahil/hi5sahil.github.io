...
The news articles from theonion are all sarcastic whereas the ones from huffingpost are all non-sarcastic. Since, the intention is to understand the linguistic features - vocabulary or semantics that help us identify sarcasm than building a 100% accurate model using just the website_name as a feature, we'll not use this variable while building our model.
```python
